...
system ('clear');
system ('figlet REFF LUMINA');

$colors = new Colors();
echo PHP_EOL."------------ [".$colors->getColoredString("AUTO REFF LUMINA - APK INFO LOKER", "green")."] -------------".PHP_EOL.PHP_EOL;
...
for ($ia=1; $ia <= $totalReff; $ia++) {
    echo "----------------------- ".$colors->getColoredString("REFFERAL KE $ia", "green")." -------------------------".PHP_EOL.PHP_EOL;
    tryRegister:
    ...
    echo "[ ".date('H:i:s')." ] -> ".$colors->getColoredString("Daftar Dengan Nama $nama", "green").PHP_EOL;
    ...
    if ($token) {
        ...
        if ($profileCard) {
            ...
            if ($anuSukses) {
                echo "[ ".date('H:i:s')." ] -> ".$colors->getColoredString("Sukse Ngereff $codeReff", "green").PHP_EOL.PHP_EOL;
            } else {
